...
### 3.7 Bizum entre vosotros: con tu Atajo de siempre

Un solo apunte que **resta en la cuenta de uno y suma en la del otro**, sin que el que recibe tenga que hacer nada. Y no hace falta un Atajo nuevo ni tocar el JSON: tu Atajo de **Transferencia** de siempre ya vale — solo hay que añadir las cuentas del otro al menú de destino.

1. Edita tu Atajo de **Transferencia**.
2. En el menú donde eliges la cuenta de **destino** ("¿A qué cuenta?"), añade opciones nuevas: las cuentas de Gaby (su Imagin, su Revolut…). El texto de la opción, el que quieras ("Imagin de Gaby"); el ID, el suyo de verdad — se copia **viendo su perfil** (botón "Ver la app de Gaby"), en Cuentas, igual que copiaste los tuyos.
3. En el iPhone de ella, lo mismo al revés: tus cuentas como opciones nuevas de destino en su Atajo de Transferencia. Su JSON se queda tal cual está (con su `"perfil":"gaby"` incluido: da igual).

La app hace el resto sola: al ver una transferencia que va de una cuenta de uno a una cuenta del otro (o que toca la conjunta), la trata como **de los dos**, ponga lo que ponga el campo `perfil` que mande el Atajo. Resta en la cuenta que sale, suma en la que entra, y cada uno la ve en su app con la cuenta del otro firmada: "Revolut → Imagin · Gaby".

Un Bizum entre vosotros no cuenta como gasto ni como ingreso en los gráficos (es mover dinero de casa, no gastarlo): solo mueve los saldos, que es lo que tiene que hacer.

El Atajo es el camino para el Bizum: el formulario de transferencias de la app solo enseña tus propias cuentas como destino (si algún día lo queréis también ahí, se añade).
