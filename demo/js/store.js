...
// modulo:rutina:inicio
  listenHabitos((items) => { state.habitos = items; notify("habitos"); });
  listenHabitosHechos((items) => { state.habitosHechos = items; notify("habitosHechos"); });
// modulo:rutina:fin
// modulo:ciclo:inicio
  listenCiclos((items) => { state.ciclos = items; notify("ciclos"); });
  listenCicloNotas((items) => { state.cicloNotas = items; notify("cicloNotas"); });
// modulo:ciclo:fin
// modulo:gimnasio:inicio
  listenGymSesiones((items) => { state.gymSesiones = items; notify("gymSesiones"); });
  listenGymMetas((items) => { state.gymMetas = items; notify("gymMetas"); });
  listenGymPesos((items) => { state.gymPesos = items; notify("gymPesos"); });
  listenGymSuplementos((items) => { state.gymSuplementos = items; notify("gymSuplementos"); });
  listenGymTomas((items) => { state.gymTomas = items; notify("gymTomas"); });
// modulo:gimnasio:fin
